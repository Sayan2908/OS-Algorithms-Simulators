...
from PyQt5.QtWidgets import QApplication, QWidget, QMainWindow, QGridLayout, QLabel, QLineEdit, QMenu
from PyQt5.QtGui import QPainter, QLinearGradient, QColor, QIcon, QIntValidator
...
class MemoryBlock:
    ...
    def paint(self, painter, x, y0, w, h_scale):
        ...
        # Convert colors to integers before passing to QColor
        ...
        # Convert x, y, w, and h to integers before drawing
        painter.drawRect(int(x), int(y), int(w), int(h))

        if self.typeID() == 2:
            painter.drawText(int(x + 0.5 * w - 5), int(y + 0.5 * h + 5), f"{self.space()} MB")
...
def requestsMemories(free_spaces):
    ...
    block_min = int(np.min(free_spaces) / 3)
    block_max = int(np.max(free_spaces))
...
def runSimulationTrials(num_trials=50, memory_size=1000, block_min=10, block_max=100):
    ...
    data = {strategy[0]: [] for strategy in fit_strategies}
    ...
    for _ in range(num_trials):
        ...
        for strategy in fit_strategies:
            data[strategy[0]].append(strategy[2](free_spaces, memory_requests)[0])

    xs = np.arange(num_trials)

    for strategy in fit_strategies:
        success_rates = data[strategy[0]]
        success_rates_avg = np.average(success_rates)
        label = '%s: %s%%' % (strategy[1], round(100 * success_rates_avg, 2))
...
class SimulatorView(QWidget):
    ...
    def paintEvent(self, ev):
        ...
        painter.setRenderHint(QPainter.Antialiasing)  # Optional: Improves text rendering
        painter.setRenderHint(QPainter.SmoothPixmapTransform)  # Optional: Smooth pixmap transform
        ...
        for label, memory in zip(labels, self._memories):
            painter.drawText(int(x), 20, f"{label}: {round(100 * memory.success, 1)}%")
            memory.paint(painter, int(x), 30, int(w), int(h))
            x += w + 20
